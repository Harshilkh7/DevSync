    ...
    async function submitHandler(e) {
        ...
        try {
            ...
            setError(
                data?.errors?.[0]?.msg ||
                data?.message ||
                (data?.errors === 'Invalid credentials' ? 'Invalid email or password.' : null) ||
                err.message ||
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-indigo-900 flex items-center justify-center p-4">
            <div className="w-full max-w-md">
                <div className="text-center mb-8">
                    ...
                    <p className="text-slate-400">Sign in to your DevSync account</p>
                </div>
                ...
                <div className="bg-gradient-to-br from-slate-800/60 to-blue-900/40 backdrop-blur-sm border border-cyan-500/20 rounded-xl p-8 shadow-2xl shadow-cyan-500/10">
                    ...
                    <form onSubmit={submitHandler} className="space-y-6">
                        ...
                        <div>
                            ...
                            <input onChange={(e) => setPassword(e.target.value)} value={password} type="password" id="password" autoComplete="current-password" minLength={3} className="w-full px-4 py-3 bg-slate-700/50 border border-cyan-500/30 rounded-lg text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-cyan-400 focus:border-cyan-400 transition-all duration-200" placeholder="Enter your password" required />
                        </div>
                    ...
                    <div className="mt-6 text-center"><p className="text-slate-400">Don&apos;t have an account?{' '}<Link to="/register" className="text-blue-400 hover:text-blue-300 font-medium transition-colors">Create one</Link></p></div>
                </div>
            </div>
        </div>
    );
